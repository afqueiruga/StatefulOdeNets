...
ArrayType = Any
JaxTreeType = Union[ArrayType, Iterable['JaxTreeType'], Dict[Union[str, int], 'JaxTreeType']]
...
IntegrationScheme = Callable[[ContinuousParameters, float, RateEquation, float], JaxTreeType]


# Basis functions
def piecewise_constant(param_nodes: Iterable[JaxTreeType],
                       t: float,
...
def params_of_t_(param_nodes: Iterable[JaxTreeType],
...
def Euler(
        params_of_t: ContinuousParameters,
        x: ArrayType,
        t0: float = 0,
        f: RateEquation = None,
...
def RK4(
    ...
    x3 = x + Dt*k3
    k4 = f(params_of_t(t0+Dt-DT_OPEN_SET_EPS), x3)  # t = 1- (inside of domain)
    return x + Dt*(1.0/6.0*k1 + 1.0/3.0*k2 + 1.0/3.0*k3 + 1.0/6.0*k4)

def OdeIntegrateFast(
        params_of_t: ContinuousParameters,
        x: ArrayType,
        f: RateEquation,

...
class PostPack:
    ...
    def __init__(self, fname):
        r = re.search(r"""ARCH-16-([A-Za-z]*)-([A-Za-z]*)-([a-z4_]*)-([0-9]*)-1.0-([0-9]*)-.*-160-(None|\[.*\])""",fname)
        self.use_batch_norms = r.group(1)
        self.use_skip_init = (r.group(2)!="NoSkip")
        self.scheme = r.group(3)
        self.initial_time_d = int(r.group(4))
        self.n_steps_per = int(r.group(5))
        if r.group(6) == "None":
            self.refine = []
        else:
            self.refine = json.loads(r.group(6))

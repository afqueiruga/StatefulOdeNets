...
def get_dataset(name='FMNIST', batch_size=128, root='.', device='cpu:0'):
    if name=='CIFAR10':
        ...
        trainloader = torch.utils.data.DataLoader(trainset.to(device), 
                    batch_size=batch_size, shuffle=True, num_workers=2)
        testset = torchvision.datasets.CIFAR10(root=root+'/CIFAR10_data', 
                    train=False, download=True, transform=transform_test)
        testloader = torch.utils.data.DataLoader(testset.to(device), 
                    batch_size=batch_size, shuffle=True, num_workers=2)

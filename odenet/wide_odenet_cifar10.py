...
class ODEResNet_Wide(nn.Module):
    ...
    def __init__(self,
        ...
        if use_stitch:
            _stitch_macro = lambda _alpha, _beta, _width : \
                ODEStitch(_alpha, _beta, _width,
                          epsilon=time_epsilon,
                          use_batch_norms=use_batch_norms,
                          use_skip_init=use_skip_init)
        else:
        ...
        self.net = NoSequential(
            ...
            nn.ReLU(),
            _macro(ALPHA, 2*ALPHA),
            _stitch_macro(ALPHA, 2*ALPHA, 2*ALPHA),
            _macro(2*ALPHA, 4*ALPHA),
            _stitch_macro(2*ALPHA, 4*ALPHA, 4*ALPHA),
            _macro(4*ALPHA, 8*ALPHA),
